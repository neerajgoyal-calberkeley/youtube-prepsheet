...
(function () {
  ...
  function buildShareableHtml(title, inner) {
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${escapeHTML(title)}</title><style>${CHEATSHEET_PRINT_CSS}</style></head><body><div class="cs-banner"><h1>${escapeHTML(title)}</h1><p>Generated by YouTube Summarizer</p></div>${inner}</body></html>`;
  }
